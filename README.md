...
Imagine we have two training sets, one with paired image and text, and one with paired text and audio. How do we train a multimodal model that also works with paired image and audio, or with all three modalities aligned simultaneously? Here we introduce commutative and transitive pre-training: 
